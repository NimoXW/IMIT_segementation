...
COPY --chown=algorithm:algorithm nnunet.zip /opt/algorithm/
...
RUN python -c "import zipfile; zipfile.ZipFile('/opt/algorithm/nnunet.zip').extractall('/opt/algorithm/')"

# nnUNet specific setup
RUN mkdir -p /opt/algorithm/nnUNet_raw_data_base/nnUNet_raw_data/Task602_ruijin_160/imagesTs
RUN mkdir -p /opt/algorithm/nnUNet_raw_data_base/nnUNet_raw_data/Task602_ruijin_160/result
